...
namespace Interact.Controls
{
    ...
    public class TextToSpeechOnChange : MonoBehaviour
    {
        ...
        private TextToSpeech mSpeechManager;
        //timer
        ...
        void Update()
        {
            ...
            if (counter >= Interval)
            {
                ...
                if ((mCachedLabel != Label.text && mInited) || mChanged)
                {
                    ...
                    mSpeechManager = GetComponentInParent<TextToSpeech>();

                    if (mSpeechManager == null)
                    {
                        // find the TextToSpeechManager somewhere else
                        mSpeechManager = FindObjectOfType<TextToSpeech>();
                    }

                    if (mSpeechManager != null)
                    {
                        // should we speak?
                        bool speak = (!mSpeechManager.IsSpeaking() || Interrupt) && !mSpeechManager.AudioSource.mute;

                        // send text to the SpeechManager
                        if (mSpeechManager != null && speak)
                        {
                            if (ToolTip == "")
                            {
                                mSpeechManager.StartSpeaking(Label.text);
                            }
                            else
                            {
                                mSpeechManager.StartSpeaking(ToolTip + " " + Label.text);
